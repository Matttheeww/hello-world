...
export const InfoStandWidgetFurniView: FC<InfoStandWidgetFurniViewProps> = props =>
{
    ...
    /*
     * Furniture position / rotation / state
     */
    const [ furniX, setFurniX ] = useState<number>(0);
    const [ furniY, setFurniY ] = useState<number>(0);
    const [ furniZ, setFurniZ ] = useState<number>(0);
    const [ furniRotation, setFurniRotation ] = useState<number>(0);
    const [ furniState, setFurniState ] = useState<number>(0);
    ...
    useEffect(() =>
    {
        ...
        /*
         * Default furniture values
         */
        let furniPositionX = 0;
        let furniPositionY = 0;
        let furniPositionZ = 0;
        let furniRotationValue = 0;
        let furniStateValue = 0;
        ...
        if(isValidController || avatarInfo.isOwner || avatarInfo.isRoomOwner || avatarInfo.isAnyRoomController)
        {
            ...
            if(avatarInfo.roomControllerLevel >= RoomControllerLevel.MODERATOR)
            {
                godMode = true;
            }
        }
        ...
        if((((avatarInfo.usagePolicy === RoomWidgetFurniInfoUsagePolicyEnum.EVERYBODY) ||
        ...
        if(godMode && roomSession)
        {
            ...
            if(roomObject)
            {
                ...
                if(location)
                {
                    furniPositionX = location.x;
                    furniPositionY = location.y;
                    furniPositionZ = location.z;
                }

                /*
                 * Get furniture rotation
                 *
                 * Nitro uses the X direction for the horizontal
                 * furniture direction/rotation.
                 */
                const direction = roomObject.getDirection();

                if(direction)
                {
                    furniRotationValue = direction.x;
                }

                /*
                 * Get furniture state
                 */
                furniStateValue = roomObject.getState(0);

                /*
                 * Existing furniture custom variables
                 */
                ...
                if(customVariables && customVariables.length)
                {
                    for(const customVariable of customVariables)
                    {
                        ...
                        customValuess.push(
                            (furnitureData && furnitureData[customVariable])
                                ? furnitureData[customVariable]
                                : ''
                        );
                    }
                }
        ...
        if(avatarInfo.isOwner || avatarInfo.isAnyRoomController)
        {
            pickupMode = PICKUP_MODE_FULL;
        }
         ...
         * Furniture position / rotation / state
         */
        setFurniX(furniPositionX);
        setFurniY(furniPositionY);
        setFurniZ(furniPositionZ);
        setFurniRotation(furniRotationValue);
        setFurniState(furniStateValue);
        ...
        if(avatarInfo.groupId)
        {
            SendMessageComposer(new GroupInformationComposer(avatarInfo.groupId, false));
        }
    ...
    const processButtonAction = useCallback((action: string) =>
    {
        ...
        switch(action)
        {
            ...
            case 'save_custom_variables':
            {
                ...
                SendMessageComposer(
                    new SetObjectDataMessageComposer(
                        avatarInfo.id,
                        map
                    )
                );
                            ...
                            { (avatarInfo.stuffData.rarityLevel > -1) &&
                                <div className="position-absolute end-0">
                                    <LayoutRarityLevelView
                                        level={ avatarInfo.stuffData.rarityLevel }
                                    />
                                </div> }
                            ...
                            <>
                                ...
                                <Text variant="white" small wrap>
                                    { LocalizeText(
                                        ...
                                        [ 'hits', 'target' ],
                                        [ crackableHits.toString(), crackableTarget.toString() ]
                                    ) }
                                </Text>
                            </> }
...
<Column gap={ 1 }>
    <Flex gap={ 1 }>
        <Column gap={ 0 } className="flex-grow-1">
    ...
    <Flex gap={ 1 }>
        ...
        <Column gap={ 0 } className="flex-grow-1">
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={ event => onFurniSettingChange(
                                                                index,
                                                                event.target.value
                                                            )}
                                                        />
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <input
                                                    ...
                                                    onChange={ event => onCustomVariableChange(
                                                        index,
                                                        event.target.value
                                                    )}
                                                />
            ...
            <Flex gap={ 1 } justifyContent="end">
                ...
                { ((furniKeys.length > 0 && furniValues.length > 0) &&
                    (furniKeys.length === furniValues.length)) &&
                    <Button
                        variant="dark"
                ...
                { ((customKeys.length > 0 && customValues.length > 0) &&
                    (customKeys.length === customValues.length)) &&
                    <Button
                        variant="dark"
